...
public:
    ...
    bool WriteZRADARSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZRADARSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZRADARSeed_deprecated(uint256& seed);
    bool EraseZRADARSeed();
    bool EraseZRADARSeed_deprecated();

    bool WriteZRADARCount(const uint32_t& nCount);
    bool ReadZRADARCount(uint32_t& nCount);
