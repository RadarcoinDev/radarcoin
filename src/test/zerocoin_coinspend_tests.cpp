...
BOOST_AUTO_TEST_CASE(zerocoin_wrapped_serial_spend_test)
{
    ...
    CzRADARWallet *czRADARWallet = new CzRADARWallet(wallet.strWalletFile);
    ...
    for (unsigned int i = 0; i < TESTS_COINS_TO_ACCUMULATE; i++) {
        ...
        czRADARWallet->GenerateDeterministicZRADAR(denom, coin, dMint, true);
        czRADARWallet->UpdateCount();
