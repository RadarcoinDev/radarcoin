...
// The zRADAR block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zRADAR is a hash of the serial
...
{
    //Create an output returning the zRADAR that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZRADAROutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zRADAR output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zRADAR", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZRADAROutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zRADAR output", __func__);
...
{
    CzRADARTracker* zradarTracker = pwallet->zradarTracker.get();
    CMintMeta meta;
    if (!zradarTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zradarTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a RADAR stake is the outpoint
