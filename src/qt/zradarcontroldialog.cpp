...
#include "ui_zradarcontroldialog.h"
...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->zradarTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zradarTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zRADAR.";
            else if (!mint.isSeedCorrect)
                strReason = "The zRADAR seed used to mint this zRADAR is not the same as currently hold in the wallet";
            else
