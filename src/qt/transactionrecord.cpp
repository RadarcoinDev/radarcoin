...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.HasZerocoinSpendInputs()) {
        ...
        if (wtx.HasZerocoinPublicSpendInputs()) {
            ...
            if (!ZRADARModule::ParseZerocoinPublicSpend(wtx.vin[0], wtx, state, publicSpend)){
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.HasZerocoinSpendInputs() && (fZSpendFromMe || wallet->zradarTracker->HasMintTx(hash))) {
            //zRADAR stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZRADAR;
...
bool IsZRADARType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZRADAR:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakeZRADAR || type == TransactionRecord::MNReward) {
