...
CzRADARWallet::CzRADARWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zradar seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZRADARSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZRADARSeed_deprecated()) {
                    LogPrintf("%s: Updated zRADAR seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zradar seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zradar wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzRADARWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZRADARCount(nCountLastUsed);
    else if (!walletdb.ReadZRADARCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzRADARWallet::Lock()
{
    seedMaster = 0;
}

void CzRADARWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzRADARWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZRADAR(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzRADARWallet::LoadMintPoolFromDB()
{
...
void CzRADARWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzRADARWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzRADARWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zradarTracker->HasPubcoinHash(pMint.first)) {
...
bool CzRADARWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZRADAR(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zradarTracker which also adds to database
    pwalletMain->zradarTracker->Add(dMint, true);
...
void CzRADARWallet::SeedToZRADAR(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzRADARWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzRADARWallet::UpdateCount()
{
    nCountLastUsed++;
    CWalletDB walletdb(strWalletFile);
    walletdb.WriteZRADARCount(nCountLastUsed);
...
void CzRADARWallet::GenerateDeterministicZRADAR(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzRADARWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzRADARWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzRADARWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
