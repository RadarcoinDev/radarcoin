...
CzRADARTracker::CzRADARTracker(std::string strWalletFile)
{
...
CzRADARTracker::~CzRADARTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzRADARTracker::Init()
{
...
bool CzRADARTracker::Archive(CMintMeta& meta)
{
...
bool CzRADARTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzRADARTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzRADARTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzRADARTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzRADARTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzRADARTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzRADARTracker::GetSerialHashes()
{
...
CAmount CzRADARTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzRADARTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzRADARTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzRADARTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzRADARTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzRADARTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzRADARTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzRADARTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzRADARTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzRADARTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzRADARTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzRADARWallet* zRADARWallet)
{
    bool iszRADARWalletInitialized = (NULL != zRADARWallet);
    CMintMeta meta;
    ...
    if (! iszRADARWalletInitialized)
        zRADARWallet = new CzRADARWallet(strWalletFile);
    meta.isSeedCorrect = zRADARWallet->CheckSeed(dMint);
    if (! iszRADARWalletInitialized)
        delete zRADARWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzRADARTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzRADARTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzRADARTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzRADARTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzRADARTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzRADARTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzRADARWallet* zRADARWallet = new CzRADARWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zRADARWallet);
        }
        delete zRADARWallet;
        LogPrint("zero", "%s: added %d dzradar from DB\n", __func__, listDeterministicDB.size());
    }
